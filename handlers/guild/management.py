...
# (VERSÃO RPG COMPLETA: LISTA LIMPA -> PERFIL -> MENU DE AÇÕES + CONVITES/LOGO/TRANSFER)
...
async def show_member_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Verifica permissões do ATOR sobre o ALVO
    ...
    # Ações de Gestão (Só aparecem se tiver permissão)
    if can_promote:
    ...
    # Se for o próprio usuário vendo seu perfil
...
async def show_rank_selection_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Lógica de quais botões mostrar baseado na hierarquia
    # General (Vice) - Valor 3
    if actor_val > 3: 
...
async def perform_rank_change(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if success:
        ...
        # Volta para o perfil do membro atualizado
...
async def warn_kick_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    name = pdata.get("character_name", "Membro")
    ...
    await query.edit_message_caption(
        caption=f"⚠️ <b>EXPULSAR MEMBRO?</b>\n\nVocê tem certeza que deseja remover <b>{name}</b> do clã? Esta ação não pode ser desfeita.",
...
async def warn_leave_clan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = [[InlineKeyboardButton("✅ Sair", callback_data='clan_leave_perform'), InlineKeyboardButton("❌ Ficar", callback_data='clan_menu')]]
    await query.edit_message_text("⚠️ <b>SAIR DO CLÃ?</b>\nVocê perderá os benefícios imediatamente.", reply_markup=InlineKeyboardMarkup(kb), parse_mode="HTML")
...
async def do_leave_clan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    except Exception as e: await query.answer(f"Erro: {e}")
...
async def start_invite_conversation(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    msg = await context.bot.send_message(query.message.chat.id, "✉️ <b>CONVITE DE CLÃ</b>\n\nDigite o <b>Nome do Personagem</b> que deseja convidar:", parse_mode="HTML")
...
async def receive_invitee_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # target é uma tupla (id_str, dict_data) ou None
    ...
    if target_chat:
        try:
            ...
            await context.bot.send_message(chat_id=update.effective_chat.id, text=f"✅ Convite enviado para {target_name}.", parse_mode="HTML")
        except: 
            await context.bot.send_message(chat_id=update.effective_chat.id, text=f"⚠️ Não foi possível contatar {target_name} (chat privado fechado?).")
    return ConversationHandler.END
...
async def accept_invite_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # Adiciona diretamente pois é convite
...
async def start_transfer_leadership(u, c) -> int:
    ...
    msg = await c.bot.send_message(q.message.chat.id, "👑 <b>NOVO LÍDER</b>\n\nDigite o <b>Nome do Personagem</b> do membro para passar a coroa:", parse_mode="HTML")
...
async def warn_delete_clan(u, c):
    q = u.callback_query; await q.answer()
    kb = [[InlineKeyboardButton("🔥 DELETAR TUDO", callback_data='clan_delete_confirm'), InlineKeyboardButton("🔙 Cancelar", callback_data='clan_manage_menu')]]
    await q.edit_message_text("⚠️ <b>DISSOLVER CLÃ?</b>\n\nIsso apagará o clã, o banco e todos os registros permanentemente.", reply_markup=InlineKeyboardMarkup(kb), parse_mode="HTML")
...
async def start_logo_conversation(u, c) -> int:
    ...
    msg = await c.bot.send_message(q.message.chat.id, "🖼️ <b>NOVO LOGO</b>\n\nEnvie uma <b>FOTO</b> ou <b>GIF</b> para ser o estandarte do clã:", parse_mode="HTML")
    c.user_data['last_bot_msg_id'] = msg.message_id
    return ASKING_LOGO
