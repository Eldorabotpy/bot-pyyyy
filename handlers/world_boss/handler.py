...
ADMIN_PLAYER_ID = "COLOQUE_AQUI_O_OBJECTID_DO_ADMIN"  # ObjectId STRING
...
# ============================================================================
# FORMATADORES E HELPERS DE ID (OBJECTID ONLY)
# ============================================================================

async def _get_boss_user_id(update: Update, context: ContextTypes.DEFAULT_TYPE) -> str | None:
    """
    Retorna o ID correto do jogador como STRING (ObjectId).
    """
...
async def wb_return_to_map(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_id = await _get_boss_user_id(update, context)
...
async def wb_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("wb_act:") or data.startswith("wb_skill:"):
        ...
        if res.get("game_over"):
            kb_die = [[
                InlineKeyboardButton("🔙 𝕄𝕖𝕟𝕦 𝕕𝕠 𝔹𝕠𝕤𝕤", callback_data='wb_menu'),
                InlineKeyboardButton("🌍 𝐌𝐀𝐏𝐀", callback_data='wb_return_map')
            ]]
