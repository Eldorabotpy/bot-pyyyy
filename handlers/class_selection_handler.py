...
async def confirm_class_choice(update: Update, context: ContextTypes.DEFAULT_TYPE, class_key: str):
    ...
    # Salva (Síncrono localmente, depois salva async)
    disp_name = entry["display_name"]
    player_data['class'] = disp_name
    player_data['class_key'] = entry["key"]
    ...
    # Recarrega os dados do jogador APÓS salvar, se necessário (mas show_kingdom_menu fará isso)
...
async def class_selection_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if query:
        # Não responder aqui ainda, deixa as funções específicas responderem
        # await query.answer()
        pass
    else: # Ignora se não for callback
         return
    ...
    # Se chegou aqui, é um callback desconhecido ou no-op, responde silenciosamente
    if query:
