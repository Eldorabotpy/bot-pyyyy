...
async def find_player_by_name(name: str) -> Optional[Tuple[str, dict]]:
    """
    Busca jogador por Nome ou Username.
    CORREÇÃO: Usa 'users_collection is not None' para evitar crash do PyMongo.
    """
    ...
    # Procura 'Bastos_af' nos campos de username
    query_user = {
        "$or": [
    ...
    # 3. TENTATIVA: BUSCA PARCIAL (Regex no Nome ou Username)
    # Se nada funcionou, procura qualquer coisa que CONTENHA o texto
    try:
...
async def find_players_by_name_partial(query: str) -> list:
    ...
    # Busca parcial simples apenas no nome visual
...
# --- CRUD e UTILS (Com Correção do 'is None') ---
