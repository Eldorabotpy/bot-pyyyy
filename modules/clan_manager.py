...
# (VERSÃO FINAL COMPLETA: PERMISSÕES HIERÁRQUICAS, SET_RANK, LIMPEZA DE MEMBROS/PEDIDOS E FUNÇÕES DE GERENCIAMENTO)
...
CLAN_RANKS: Dict[str, Dict[str, Any]] = {
...
try:
    ...
    try:
        ...
        logger.warning(f"⚠️ [CLAN MANAGER] Não foi possível criar índice name_lower: {_e}")
...
async def get_clan(clan_id: str) -> Optional[dict]:
    if clans_col is None or not clan_id:
        return None
...
async def get_member_rank(clan: dict, user_id: str) -> str:
    ...
    ranks = clan.get("member_ranks", {}) or {}
...
async def check_permission(clan: dict, actor_id: str, action: str, target_id: str = None) -> bool:
    ...
    if target_id is not None:
        ...
        # Ninguém pode afetar a si mesmo aqui
        ...
        if action == "kick":
            # Se passou pela regra de ouro, está permitido
            return True
...
async def set_member_rank(clan_id: str, actor_id: str, target_id: str, new_rank_key: str) -> Tuple[bool, str]:
    ...
    actor_id = _ensure_str(actor_id)
    target_id = _ensure_str(target_id)
    new_rank_key = _ensure_str(new_rank_key)
...
# ==============================================================================
# LIMPEZA: REMOVER MEMBROS INVÁLIDOS / PEDIDOS INVÁLIDOS
# ==============================================================================
async def cleanup_pending_applications(clan_id: str) -> Tuple[int, List[str]]:
    """
    Remove candidaturas pendentes cujo jogador NÃO existe mais (não migrou / inválido).
    Retorna (qtde_removida, lista_ids_removidos).
    """
    if clans_col is None or users_col is None:
        return 0, []
    ...
    pending_raw = clan.get("pending_applications", []) or []
    pending = [_ensure_str(x) for x in pending_raw]

    removed: List[str] = []
    for pid in pending:
        exists = users_col.find_one({"_id": pid}, {"_id": 1})
        if not exists:
            removed.append(pid)
    ...
    clans_col.update_one(
        {"_id": clan_id},
        {"$pull": {"pending_applications": {"$in": removed}}}
    )

    return len(removed), removed

async def cleanup_invalid_members(clan_id: str) -> Tuple[int, List[str]]:
    """
    Remove membros inválidos (player não existe no banco novo).
    Nunca remove o líder.
    Também limpa member_ranks desses ids.
    """
    if clans_col is None or users_col is None:
        return 0, []
    ...
    leader_id = _ensure_str(clan.get("leader_id"))
    members_raw = clan.get("members", []) or []
    members = [_ensure_str(x) for x in members_raw]

    removed: List[str] = []
    for mid in members:
        if mid == leader_id:
            continue
        exists = users_col.find_one({"_id": mid}, {"_id": 1})
        if not exists:
            removed.append(mid)
    ...
    update = {
        ...
        "$unset": {f"member_ranks.{rid}": "" for rid in removed}
    }
    clans_col.update_one({"_id": clan_id}, update)
...
async def create_clan(leader_id: Union[str, int], clan_name: str, payment_method: str = "gold") -> str:
    if clans_col is None:
        raise ValueError("Banco de dados offline.")
    ...
    new_clan = {
        ...
        "member_ranks": {},  # {user_id: "vice", user_id2: "elder"}
        "max_members": 10,
...
async def add_application(clan_id: str, user_id: Union[str, int]):
    ...
    members = [_ensure_str(m) for m in (clan.get("members", []) or [])]
    if user_id_str in members:
        raise ValueError("Você já é membro deste clã.")

    pending = [_ensure_str(p) for p in (clan.get("pending_applications", []) or [])]
...
async def accept_application(clan_id: str, applicant_id: Union[str, int]):
    ...
    clan = await get_clan(clan_id)
    if not clan:
        raise ValueError("Clã inválido.")

    if len(clan.get("members", []) or []) >= (clan.get("max_members", 10) or 10):
...
async def transfer_leadership(clan_id: str, old_leader_id: Union[str, int], new_leader_id: Union[str, int]):
    ...
    members_str = [_ensure_str(m) for m in (clan.get("members", []) or [])]
...
async def promote_member(clan_id: str, actor_id: str, target_id: str) -> Tuple[bool, str]:
    ...
    if target_rank_key == "member":
        next_rank = "elder"
    elif target_rank_key == "elder":
        next_rank = "vice"
    elif target_rank_key == "vice":
        return False, "Cargo máximo atingido! Transfira a liderança."
    elif target_rank_key == "leader":
...
async def demote_member(clan_id: str, actor_id: str, target_id: str) -> Tuple[bool, str]:
    ...
    prev_rank = None
    if target_rank_key == "vice":
        prev_rank = "elder"
    elif target_rank_key == "elder":
        prev_rank = "member"
    elif target_rank_key == "member":
        return False, "Já está no cargo mais baixo."
    elif target_rank_key == "leader":
        return False, "Não pode rebaixar o líder."
...
async def bank_deposit(clan_id: str, user_id: Union[str, int], amount: int) -> Tuple[bool, str]:
    ...
    try:
        ...
        try:
            ...
            if pdata:
                player_name = pdata.get("character_name", player_name)
        except Exception:
            pass
...
async def level_up_clan(clan_id: str, user_id: str, payment_method: str):
    ...
    # Compatibilidade: tenta do próximo nível; se não existir, do atual
    req_pts = next_lvl_info.get("points_to_next_level")
    if req_pts is None:
        req_pts = CLAN_PRESTIGE_LEVELS.get(current_lvl, {}).get("points_to_next_level", 999999)

    cost = next_lvl_info.get("upgrade_cost", {}) or {}
    if current_pts < req_pts:
        raise ValueError("Faltam pontos de prestígio.")
    ...
    if payment_method == "gold":
        ...
        update_query = {
            "$inc": {"bank": -cost_val, "prestige_level": 1, "prestige_points": -req_pts},
            "$set": {"max_members": next_lvl_info.get("max_members", 10)}
        }
    elif payment_method == "dimas":
        update_query = {
            "$inc": {"prestige_level": 1, "prestige_points": -req_pts},
            "$set": {"max_members": next_lvl_info.get("max_members", 10)}
        }
    else:
...
async def update_guild_mission_progress(user_id: Union[str, int], action_type: str, target_id: str, quantity: int = 1):
    ...
    action_type_u = str(action_type).upper()

    match = False
    if mission_type == "HUNT" and action_type_u == "HUNT":
        if mission.get("target_monster_id") == target_id:
            match = True
    elif mission_type == "COLLECT" and action_type_u == "COLLECT":
        if mission.get("target_item_id") == target_id:
            match = True

    if match:
        clans_col.update_one({"_id": clan_id}, {"$inc": {"active_mission.current_progress": quantity}})
...
async def delete_clan(clan_id: str, leader_id: Union[str, int]):
    ...
    if users_col is not None:
        users_col.update_many({"clan_id": clan_id}, {"$set": {"clan_id": None}})
