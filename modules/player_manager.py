...
# (VERSÃO CORRIGIDA: STRICT OBJECTID + wrappers consistentes)
...
from .player.queries import (
    ...
    iter_players,
    iter_player_ids,  # essencial para jobs/handlers
)
...
# ==============================================================================
def _ensure_id_format(user_id: Union[str, ObjectId, None]) -> Optional[ObjectId]:
    """
    Normaliza o identificador do jogador para ObjectId.
    - Aceita ObjectId diretamente.
    - Aceita string válida de ObjectId.
    - Rejeita qualquer outro tipo/valor (retorna None).
    """
    if user_id is None:
        logger.warning("❌ [ID ERROR] user_id=None")
        return None
    ...
    if isinstance(user_id, str):
        ...
        logger.warning(f"❌ [ID ERROR] String não é ObjectId válido: {user_id!r}")
        return None

    logger.warning(f"❌ [ID ERROR] Tipo de ID inválido (esperado str/ObjectId): {type(user_id)}")
    return None
...
# 3. WRAPPERS E LÓGICA
# ==============================================================================
async def get_player_data(user_id: Union[str, ObjectId, None]):
...
async def save_player_data(user_id: Union[str, ObjectId, None], data: dict):
    real_id = _ensure_id_format(user_id)
    if not real_id:
        return None
...
def spend_energy(player_data: dict, amount: int) -> bool:
    """
    Gasta energia do player_data em memória e dispara atualização de missão em background.
    IMPORTANTE: Mantém referência 100% ObjectId.
    """
    success = _spend_energy_internal(player_data, amount)
    if success and amount > 0:
        try:
            raw_id = player_data.get("_id") or player_data.get("user_id")
            real_id = _ensure_id_format(raw_id)
            ...
            try:
                asyncio.get_running_loop().create_task(_bg_mission())
            except Exception:
                pass
        except Exception:
            pass
...
async def safe_add_gold(user_id: Union[str, ObjectId, None], amount: int) -> int:
...
async def safe_spend_gold(user_id: Union[str, ObjectId, None], amount: int) -> bool:
    pdata = await get_player_data(user_id)
    if not pdata:
        return False
...
async def full_restore(user_id: Union[str, ObjectId, None]):
    pdata = await get_player_data(user_id)
    if pdata:
