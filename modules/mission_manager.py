...
def _normalize_player_id(pid: PlayerId) -> ObjectId:
...
def get_player_missions(player_id: PlayerId) -> dict:
    """Retorna missões apenas para leitura."""
    return {}


async def update_mission_progress(player_id: PlayerId, action_type: str, target_id: str, quantity: int = 1):
    """
    Central de processamento de missões com logs detalhados.
    """
    try:
        oid = _normalize_player_id(player_id)
    except ValueError:
        return []

    # [DEBUG]
    print(f"\n--- [MISSION DEBUG] Tentando atualizar: Player={oid}, Action={action_type}, Target={target_id}, Qty={quantity} ---")

    player_data = await player_manager.get_player_data(oid)
    if not player_data:
    ...
    for m in missions:
        ...
        mission_type = str(m.get("type", "")).upper()  # No save pode estar lowercase
        req_action = str(action_type).upper()          # Padroniza para UPPER
        ...
        # --- APLICA O PROGRESSO ---
        if match:
            current = int(m.get("progress", 0))
            req = int(m.get("target_count", m.get("qty", 1)))  # target_count ou fallback qty

            if current < req:
                increment = min(int(quantity), req - current)
                m["progress"] = current + increment
                updated = True

                # Verifica conclusão
                if int(m["progress"]) >= req:
                    m["status"] = "completed"
                    # logs.append(f"✅ Missão Concluída: <b>{m.get('title', 'Missão')}</b>!")

    if updated:
        player_data["adventurer_guild"] = gdata
        await player_manager.save_player_data(oid, player_data)
            ...
            if active_mission:
                ...
                if not completed:
                    ...
                    if match_clan:
                        ...
                        db.clans.update_one(
                            {"_id": clan_id},
                            {"$inc": {"active_mission.current_progress": int(quantity)}}
                        )
                    else:
...
async def claim_personal_reward(player_id: PlayerId, mission_index: int):
    """
    ...
    points = int(rewards.get("prestige_points", 0))

    if gold > 0:
        player_manager.add_gold(player_data, gold)
    if xp > 0:
        player_data["xp"] = int(player_data.get("xp", 0)) + xp

    # Pontos de Guilda (Rank)
    gdata["points"] = int(gdata.get("points", 0)) + points
...
def reroll_mission(player_id, index):
    return False
